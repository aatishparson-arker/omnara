...
func (s *Store) ExchangeOAuthAuthorizationCode(
	...
	code, err := qtx.ConsumeOAuthAuthorizationCode(
		ctx,
		dbsqlc.ConsumeOAuthAuthorizationCodeParams{CodeHash: HashBearerToken(input.Code)},
	)
	...
	if err := lockActiveUser(ctx, qtx, code.UserID); err != nil {
...
func (s *Store) RefreshOAuthAccessToken(
	...
	if input.Resource != "" && rotated.Resource != input.Resource {
		return OAuthTokenSetRecord{}, storeerr.ErrUnauthorized
	}
	if err := lockActiveUser(ctx, qtx, rotated.UserID); err != nil {
		return OAuthTokenSetRecord{}, err
	}
	if err := tx.Commit(ctx); err != nil {
...
func lockActiveUser(ctx context.Context, qtx *dbsqlc.Queries, userID ID) error {
